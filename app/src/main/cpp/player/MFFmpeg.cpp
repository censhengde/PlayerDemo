...
void *task_prepare(void *args) {
    MFFmpeg *ffmpeg = static_cast<MFFmpeg *>(args);
    ffmpeg->_prepare();
    return 0;
}

MFFmpeg::MFFmpeg(JavaCallHelper *callHelper, const char *dataSource) {
...
void MFFmpeg::prepare() {
    //创建一个线程
    pthread_create(&pid, 0, task_prepare, this);
}

void MFFmpeg::_prepare() {
    // 初始化网络 让ffmpeg能够使用网络
...
void MFFmpeg::setRenderFrameCallback(RenderFrameCallback callback) {
